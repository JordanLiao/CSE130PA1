...
const fold_left = function (f, base, ls) {
  ...
  // Write the recursive fold_left call
	var newBase = f(base, ls.get(0));
  return fold_left(f,newBase, ls.shift());
};
...
const map = (g, ls) => {
	const empls = List();
	const f = function(acc, elem) {
		const temp = g(elem);
		return acc.push(temp);
	}
	return fold_left(f, empls, ls);
}
...
const partition = (g, ls) => {
	...
	const f = function(acc, elem) {
		if(g(elem)){
			let temp = acc.get(0).push(elem);
			return acc.set(0, temp);
		}
		else{
			let temp = acc.get(1).push(elem);
			return acc.set(1, temp);
		}
	}
	return fold_left(f,lst,ls);
}
...
const quicksort = function(ls) {
  if(ls.size <= 1) {
	...
	let ls1 = ls.shift();
	let lis = partition(f,ls1);
	let lft = (quicksort(lis.get(0))).sort();
	let rht = (quicksort(lis.get(1))).sort();
	let result = lft.push(pivot);
	return result.concat(rht);
};
...
//const lss = List([3,6,2,8,3,2]);
//const result = quicksort(lss);
